...
onMounted(async() => {
  await obtenerDatos();
   ...
   const ctx = document.getElementById('portafolioChart');
  const data = {
    labels: accionesSimbolo,
    datasets: [{
      label: '',
      data: porcentajesAcciones,
      backgroundColor: [
...
const { obtenerDatos, porcentajesAcciones, accionesSimbolo } = useServices();
